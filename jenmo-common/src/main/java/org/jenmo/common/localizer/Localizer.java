...
/**
 * The Localizer provides convenient access to localized strings. It includes built-in support for
 * parameter substitution through the use of the {@link MessageFormat} utility. Strings are stored
 * in per-package {@link Properties} files. The property file for the default locale must be named
 * <code>localizer.properties</code>. Additional locales can be supported through additional
 * property files using the naming conventions specified in the {@link ResourceBundle} class. For
 * example, the german locale could be supported through a <code>localizer_de_DE.properties</code>
 * file.
 * 
...
public class Localizer {
   ...
   private static final Collection<ResourceBundleProvider> _providers = new CopyOnWriteArraySet<ResourceBundleProvider>(
         Arrays.asList(new ResourceBundleProvider[] { new SimpleResourceBundleProvider() }));

   /**
    * Return a Localizer instance that will access the properties file in the package of the given
    * class using the system default locale.
    * 
   ...
   /**
    * Return a Localizer instance that will access the properties file in the package of the given
    * class using the given locale.
    * 
    * @param cls
    *           the class whose package to check for the localized properties file; if null, the
    *           system will check for a top-level properties file
    * @param locale
    *           the locale to which strings should be localized; if null, the system default will be
    *           assumed
    */
   public static Localizer forPackage(Class<?> cls, Locale locale) {
      ...
      if (loc != null)
         ...
         loc = new Localizer(pkg, file, locale, cls == null ? null : (ClassLoader) AccessController
               .doPrivileged(J2DoPrivUtils.getClassLoaderAction(cls)));
   ...
   private ResourceBundle getBundle() {
      ...
      if (_bundle == null) {
         ...
         for (Iterator<ResourceBundleProvider> itr = _providers.iterator(); itr.hasNext()
               && _bundle == null;) {
   ...
   /**
    * Return the localized string matching the given key. The given <code>sub</code> object will be
    * packed into an array and substituted into the found string according to the rules of the
   ...
   /**
    * Return the localized string matching the given key. The given <code>subs</code> objects will
    * be substituted into the found string according to the rules of the {@link MessageFormat}
    * class.
    * 
    * @see #get(String)
    */
   ...
   /**
    * A <code>Message</code> can provide a localized message via the {@link #getMessage} method
    * call, and can also provide the original key, package, and substitution array that were used to
   ...
   public static class Message {
      ...
      private Message(String packageName, ResourceBundle bundle, String key, Object[] subs,
            boolean fatal) {
